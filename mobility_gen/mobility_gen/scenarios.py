...
@SCENARIOS.register()
class GoToFixedPointScenario(Scenario):
    def __init__(self, 
            robot: Robot, 
            occupancy_map: OccupancyMap
        ):
    ...
    def set_target_path(self):
        ...
        # Convert current pose to pixels
        ...
        start = (int(start_px[0, 1]), int(start_px[0, 0]))  # y, x

        # Fixed target (X=3.8, Y=-9.0)
        goal_px = self.occupancy_map.world_to_pixel_numpy(np.array([[3.8, -9.0]]))
        goal = (int(goal_px[0, 1]), int(goal_px[0, 0]))

        # Plan path
        output = generate_paths(start, freespace)
        path = output.unroll_path(goal)
    ...
    def step(self, step_size: float):
        ...
        # Control law (same as RandomPathFollowingScenario)
        ...
        angular_velocity = - self.robot.path_following_angular_gain * d_theta
        self.robot.action.set_value(np.array([linear_velocity, angular_velocity]))
